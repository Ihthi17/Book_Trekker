...
?>
    ...
    left JOIN categories ON categories.c_id = book.category_id
    WHERE `b_name` LIKE '%$search_query%' OR `keyword` LIKE '%$search_query%'";
    $result = $con->query($sql);
    ...
    elseif (isset($_GET['c_id'])) {
        $category_id = $_GET['c_id']; // Get the selected category ID from the URL parameter
    
        // Build the SQL query to retrieve books for the selected category
        ...
        // Display the books for the selected category
        if ($result->num_rows > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
