    ...
    <div class="container-fluid pt-5">
        <div class="row px-xl-5">
            <div class="col-lg-8 table-responsive mb-5">
                <?php
$sql = "SELECT * FROM `order` LEFT JOIN user ON user.u_id = order.user_id where order.user_id='" . $_SESSION['u_id'] . "'";

$result = $con->query($sql);
if ($result->num_rows > 0) {
    ?>
                <center>
                    <table class="table table-dark mb-5 align-middle">
                        <tr>
                            <th>Order ID</th>
                            <th>Total</th>
                            <th>Address</th>
        ...
        ?>
                        <tr>
                            ...
                            <td>
                                <p
                                    class="text-
                                    <?php echo ($row['status']==0)? 'btn-warning':(($row['status']==1)? 'btn-primary':'btn-success'); ?>">
                                    <?php echo ($row['status']==0)? 'Pending':(($row['status']==1)? 'Packed':'Delivered');  ?>
                                </p>
                            </td>
                            <td><button class="btn btn-dark "> <a
                                        href="view_order_details.php?order_id=<?php echo $row['order_id']; ?>"
                                        class="btn btn-success ">
                                        <i class="fa-solid fa-paperclip rounded-circle">View</i>
                                    </a></button>
                            </td>
